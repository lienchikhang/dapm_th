...
const register = async (req, res) => {
    ...
    try {
        ...
        if(user) return res.status(401).json({success: false, message: "Username is already taken!"})

        const hashedPassword = crypto.AES.encrypt(password, process.env.PASS_CRYPTO);
